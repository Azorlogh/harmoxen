//! A TextInput-like widget that takes a parser.
//! It holds more internal state because
//!
...
use crate::text;
use crate::{Clipboard, Element, Hasher, Layout, Length, Point, Rectangle, Size, Widget};
	...
	Renderer: self::Renderer,
{
	...
	fn on_event(
		...
		match event {
			...
			Event::Mouse(mouse::Event::CursorMoved { x, .. }) => {
				if self.state.is_dragging {
					...
					if target > 0.0 {
						...
						self.state.cursor.select_range(self.state.cursor.start(&value), position);
					}
			...
			Event::Keyboard(keyboard::Event::KeyPressed { key_code, .. }) if self.state.is_focused => {
				...
				match key_code {
					...
					keyboard::KeyCode::Backspace => {
						if platform::is_jump_modifier_pressed(modifiers) && self.state.cursor.selection(&self.value).is_none() {
							if self.is_secure {
					...
					keyboard::KeyCode::Home => {
						if modifiers.shift {
							self.state.cursor.select_range(self.state.cursor.start(&self.value), 0);
						} else {
							self.state.cursor.move_to(0);
						}
					}
					keyboard::KeyCode::End => {
						if modifiers.shift {
							self.state
								.cursor
								.select_range(self.state.cursor.start(&self.value), self.value.len());
						} else {
					...
					keyboard::KeyCode::Escape => {
						...
						self.state.keyboard_modifiers = keyboard::Modifiers::default();
					}
					_ => {}
				}
			...
			Event::Keyboard(keyboard::Event::KeyReleased { key_code, .. }) if self.state.is_focused => {
				match key_code {
					keyboard::KeyCode::V => {
						self.state.is_pasting = None;
					}
			...
			Event::Keyboard(keyboard::Event::ModifiersChanged(modifiers)) if self.state.is_focused => {
