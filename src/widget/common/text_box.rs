...
use druid::{
	Application, BoxConstraints, Cursor, Env, Event, EventCtx, HotKey, KeyCode, LayoutCtx, LifeCycle, LifeCycleCtx, PaintCtx,
	Selector, SysMods, TimerToken, UpdateCtx, Widget,
};

use druid::kurbo::{Affine, Line, Point, RoundedRect, Size, Vec2};
use druid::piet::{FontBuilder, PietText, PietTextLayout, RenderContext, Text, TextLayout, TextLayoutBuilder};
use druid::theme;

use druid::text::{
	movement, offset_for_delete_backwards, BasicTextInput, EditAction, EditableText, MouseAction, Movement, Selection,
	TextInput,
};
...
impl TextBox {
	...
	fn caret_to(&mut self, text: &mut String, to: usize) {
		match text.cursor(to) {
			...
			None => eprintln!("You can't move the cursor there."),
		}
	}
...
impl Widget<String> for TextBox {
	fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut String, env: &Env) {
		...
		match event {
			...
			Event::Command(ref cmd) if ctx.is_focused() && (cmd.is(druid::commands::COPY) || cmd.is(druid::commands::CUT)) => {
	...
	fn paint(&mut self, ctx: &mut PaintCtx, data: &String, env: &Env) {
		...
		let content = if data.is_empty() { &self.placeholder } else { data };
		...
		ctx.with_save(|rc| {
			...
			if !self.selection.is_caret() {
				...
				let selection_rect =
					RoundedRect::from_origin_size(selection_pos, Size::new(selection_width + 2., font_size + 4.).to_vec2(), 1.);
			...
			let color = if data.is_empty() { &placeholder_color } else { &text_color };
