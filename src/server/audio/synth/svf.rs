...
fn svf_core(f: f64, res: f64, m: [f64; 3]) -> Kernel {
	let k = 2.0 - 2.0 * res;
	let g = (std::f64::consts::PI * f).tan() * 1.0;
...
pub fn lowpass(f: f64, res: f64) -> Kernel {
	svf_core(f, res, [0.0, 0.0, 1.0])
}

pub fn bandpass(f: f64, res: f64) -> Kernel {
	svf_core(f, res, [0.0, 1.0, 0.0])
}

pub fn highpass(f: f64, res: f64) -> Kernel {
	let k = 2.0 - 2.0 * res;
